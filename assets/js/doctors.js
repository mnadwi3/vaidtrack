...
(function () {
  ...
  var ICON_PERSON =
    '<svg fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.5"/><path stroke-linecap="round" stroke-linejoin="round" d="M5 20c0-3.87 3.13-7 7-7s7 3.13 7 7"/></svg>';
  ...
  // Shared by the homepage's "Top Doctors" section and the /all-doctors
  // directory page — photo-top, bordered card with a specialty badge,
  // matching the doctor-card style used on the landing microsites
  // (e.g. /knee-replacement-in-india).
  function renderDirectoryCard(doctor) {
    ...
    var photoHtml = image
      ? '<img src="' + escapeHtml(image) + '" alt="' + escapeHtml(name) + '" loading="lazy" decoding="async" class="dcard-img">'
      : '<span class="dcard-initials">' + escapeHtml(initials) + '</span>';

    return (
      '<article class="dcard reveal" data-doctor-slug="' + escapeHtml(doctor.slug || '') + '">' +
        '<div class="dcard-photo">' + photoHtml + '</div>' +
        '<div class="dcard-info">' +
          (specialty ? '<span class="dcard-badge">' + escapeHtml(specialty) + '</span>' : '') +
          '<div class="dcard-textgroup">' +
            '<h3 class="dcard-name">' + ICON_PERSON + '<span>' + escapeHtml(name) + '</span></h3>' +
            (qualification ? '<p class="dcard-creds">' + ICON_EDU + '<span>' + escapeHtml(qualification) + '</span></p>' : '') +
            (experience ? '<div class="dcard-exp">' + ICON_CLOCK + '<span>' + escapeHtml(experience) + '</span></div>' : '') +
          '</div>' +
          '<div class="dcard-actions">' +
            '<a class="dcard-cta" href="' + waUrl(bookMsg) + '" target="_blank" rel="noopener noreferrer">' +
              ICON_WA + 'Book Appointment</a>' +
          '</div>' +
        '</div>' +
      '</article>'
    );
  ...
  var HOME_FEATURED_LIMIT = 10;
  // Homepage-only content scoping: VaidTrack's core specialties are
  // oncology and cardiology (see meta description), so the homepage
  // doctor grid excludes off-brand specialties like gynecology/dermatology
  // entirely and ranks cancer/cardiac specialists above everything else.
  // The /all-doctors directory and treatment pages are unaffected.
  var HOME_EXCLUDED_SPECIALTY_RE = /gyn(a|ae)?colog|dermatolog/i;
  var HOME_PRIORITY_SPECIALTY_RE = /onco|cancer|cardio|cardiac/i;

  function homeSpecialtyText(doctor) {
    return (doctor.specialty || '') + ' ' + expertiseLabel(doctor.expertise);
  }

  function mountDoctors(doctors) {
    var homeGrid = document.querySelector('#doctors .doc-grid');
    if (homeGrid) {
      var homePool = doctors.filter(function (d) {
        return !HOME_EXCLUDED_SPECIALTY_RE.test(homeSpecialtyText(d));
      });
      var homeDoctors = homePool
        .slice()
        .sort(function (a, b) {
          var featuredDiff = (b.is_featured ? 1 : 0) - (a.is_featured ? 1 : 0);
          if (featuredDiff !== 0) return featuredDiff;
          var aPriority = HOME_PRIORITY_SPECIALTY_RE.test(homeSpecialtyText(a)) ? 0 : 1;
          var bPriority = HOME_PRIORITY_SPECIALTY_RE.test(homeSpecialtyText(b)) ? 0 : 1;
          return aPriority - bPriority;
        })
        .slice(0, HOME_FEATURED_LIMIT);
      var homeHtml = homeDoctors.map(renderDirectoryCard).join('');
    ...
    if (allGrid) {
      allGrid.innerHTML = doctors.length
        ? doctors.map(renderDirectoryCard).join('')
